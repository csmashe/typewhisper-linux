...
public static partial class DeveloperFormattingService
{
    ...
    {
        text = EmailPhraseRegex()
            .Replace(
                text,
                match =>
                {
                    var domain = ReplaceRepeated(
                        match.Groups["domain"].Value,
                        DotBetweenCodeTokensRegex(),
                        "${left}.${right}"
                    );
    ...
    private static string? TryFormatCasingCommand(string text)
    {
        ...
        var words = WordRegex()
            .Matches(match.Groups["text"].Value)
    ...
    [GeneratedRegex(
        @"\b(?<left>[A-Za-z0-9._%+-]+)\s+at\s+(?<domain>[A-Za-z0-9-]+(?:\s+dot\s+[A-Za-z0-9-]+)+)\b",
        RegexOptions.IgnoreCase
    )]
    private static partial Regex EmailPhraseRegex();

    [GeneratedRegex(
        @"\b(?<left>[A-Za-z_][A-Za-z0-9_]*|\d+)\s+dot\s+(?<right>[A-Za-z_][A-Za-z0-9_]*|\d+)\b",
        RegexOptions.IgnoreCase
    )]
    private static partial Regex DotBetweenCodeTokensRegex();

    [GeneratedRegex(
        @"\b(?<left>\d+(?:\.\d+)?)\s+plus\s+(?<right>\d+(?:\.\d+)?)\b",
        RegexOptions.IgnoreCase
    )]
    private static partial Regex NumericPlusRegex();

    [GeneratedRegex(
        @"\b(?<left>\d+(?:\.\d+)?)\s+minus\s+(?<right>\d+(?:\.\d+)?)\b",
        RegexOptions.IgnoreCase
    )]
